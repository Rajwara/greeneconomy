      ...
      <div className="cards grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-4 gap-8 ">
        {data.map((item) => (
          <div
            ...
            key={item.id}
          >
            <h6 className="uppercase text-sm md:text-base lg:text-lg text-[#BAB220] font-bold leading-9	font-montserrat">
              {item.heading}
            </h6>
            <p className=" mt-4 text-[#3c3c3c] font-poppins text-sm md:text-base lg:text-lg leading-7 font-normal">
              {item.description}
            </p>
          </div>
        ))}
